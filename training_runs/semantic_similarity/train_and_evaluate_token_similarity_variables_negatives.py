...
from experimental_wsd.config import RaganatoEnglish  # noqa: E402
from experimental_wsd.data_processing.lightning_data_modules.semcor import (  # noqa: E402
    VariableSemCorTraining,
)
from experimental_wsd.evaluation.token_similarity_variable_negatives import (  # noqa: E402
    evaluate_on_wordnet_dataset,
)
...
def model_cli() -> None:
    ...
    best_model_path = str(
        Path(cli.trainer.checkpoint_callback.best_model_path).resolve()
    )
    model = TokenSimilarityVariableNegatives.load_from_checkpoint(best_model_path)
    model.eval()

    validation_datasets = [("SE7", RaganatoEnglish.semeval_2007)]
    test_datasets = [
    ...
    all_datasets = [("validation", validation_datasets), ("test", test_datasets)]
    ...
    for dataset_split_name, datasets in all_datasets:
        for dataset_name, dataset in datasets:
            micro_f1, macro_f1 = evaluate_on_wordnet_dataset(
                model,
                dataset,
                wordnet_sense_getter,
                tokenizer,
                wordnet,
                wordnet_lexicon_name,
            )
            ...
            logger.info(
                f"{dataset_split_name} dataset: {dataset_name}: "
    ...
    for ml_logger in cli.trainer.loggers:
        if isinstance(ml_logger, CSVLogger):
            ...
            with Path(logging_directory, save_data_filename).open(
                "w", encoding="utf-8"
            ) as save_fp:
