...
from lightning.pytorch.tuner.tuning import Tuner  # noqa: E402
...
def model_cli() -> None:
    ...
    lr_results = tuner.lr_find(
        cli.model, datamodule=cli.datamodule, attr_name="learning_rate"
    )
    lr_suggestion = lr_results.suggestion()
    lr_suggestion_loss = None
    learning_rate_results = lr_results.results
    sorted_learning_rates = sorted(
        zip(learning_rate_results["lr"], learning_rate_results["loss"]),
        key=lambda x: x[1],
    )
    ...
    if lr_suggestion_loss:
        logging.info(
            f"Suggested LR: {lr_suggestion}, loss at this LR: {lr_suggestion_loss}"
        )
    else:
