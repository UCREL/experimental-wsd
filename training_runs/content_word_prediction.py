...
from experimental_wsd.data_processing_utils import (
    get_pre_processed_label_statistics,
...
wsl_ds_jsonl_file_path = write_to_jsonl(
    wsl_sentence_generator(wsl_ds, "validation", word_net_sense_getter=GET_SENSE),
    DATA_PROCESSING_DIR,
    "wsl_test",
    overwrite=False,
)
...
test_is_content_word_dataset = wsl_dataset.map(
    map_token_text_and_is_content_labels,
    remove_columns=wsl_dataset["train"].column_names,
    batched=False,
)
test_labelled_content_word_dataset = test_is_content_word_dataset.map(
    tokenize_pre_processing,
    batched=True,
    fn_kwargs={"tokenizer": tokenizer},
    remove_columns=test_is_content_word_dataset["train"].column_names,
)


is_content_word_dataset = semcor_dataset_splits.map(
    map_token_text_and_is_content_labels,
    remove_columns=semcor_dataset_splits["train"].column_names,
    batched=False,
)
labelled_content_word_dataset = is_content_word_dataset.map(
    tokenize_pre_processing,
    batched=True,
    fn_kwargs={"tokenizer": tokenizer},
    remove_columns=is_content_word_dataset["train"].column_names,
)


train_label_counts = labelled_content_word_dataset.map(
    get_pre_processed_label_statistics,
    batched=True,
    fn_kwargs={"label_key": "labels", "label_value_to_ignore": -100},
    remove_columns=labelled_content_word_dataset["train"].column_names,
)
...
test_classifier = TokenClassifier(
    base_model_name,
    True,
...
train_batch_sampler = AscendingSequenceLengthBatchSampler(
    train_dataset, batch_size=128, length_key="input_ids", random=True
)
...
validation_batch_sampler = AscendingSequenceLengthBatchSampler(
    validation_dataset, batch_size=128, length_key="input_ids", random=False
)
...
test_batch_sampler = AscendingSequenceLengthBatchSampler(
    test_dataset, batch_size=128, length_key="input_ids", random=False
)
