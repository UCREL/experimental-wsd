...
def main(
    ...
    tags_that_cannot_be_predicted: Annotated[
        list[str] | None,
        typer.Option(
            "-p",
            help="Tags that should be removed from the list of possible tags that the model can predict",
        ),
    ] = None,
    tags_not_to_evaluate_on: Annotated[
        list[str] | None,
        typer.Option(
            "-e", help="Tags that should be removed from the evaluation dataset."
        ),
    ] = None,
):
    ...
    usas_tag_to_description_mapper = processed_usas_utils.load_usas_mapper(
        USASMapper, tags_that_cannot_be_predicted_set
    )
    ...
    test_processed_file_path = processed_usas_utils.process_file(
        test_file_path, DATA_PROCESSING_DIR, "variable_mosaico_usas_9"
    )
    test_dataset = datasets.load_dataset(
        "json", data_files=str(test_processed_file_path)
    )["train"].take(20000)
    model = TokenSimilarityVariableNegatives.load_from_checkpoint(best_model_path_str)
    tokenizer = AutoTokenizer.from_pretrained(
        model.base_model_name, add_prefix_space=True
    )
    ...
    with torch.inference_mode(mode=True):
        ...
        for index, usas_tag_description in enumerate(
            ...
            tokenized_usas_description = tokenizer(
                usas_description, truncation=False, padding=False, return_tensors="pt"
            )
            description_input_ids = tokenized_usas_description.input_ids.to(
                model.device
            ).unsqueeze(0)
            description_attention_mask = tokenized_usas_description.attention_mask.to(
                model.device
            ).unsqueeze(0)
            definition_embedding = model.label_definition_encoding(
        ...
        for sentence_index, sentence in enumerate(test_dataset):
            all_usas_tags, usas_token_offsets = (
                sentence["usas"],
                sentence["usas_token_offsets"],
            )
            ...
            for usas_tags, usas_token_offset in zip(all_usas_tags, usas_token_offsets):
                ...
                token_embedding = model.token_encoding_using_text_encoding(
                    text_embedding, text_word_ids_mask
                )
                label_similarity_score = model.token_label_similarity(
                    usas_embedded_descriptions_tensor, token_embedding
                )
                predicted_usas_tag = usas_index_to_tag[
                    torch.argmax(label_similarity_score).item()
                ]
