...
# The environment variables needs to be loaded before this import
from experimental_wsd.config import DATA_PROCESSING_DIR, RaganatoEnglish  # noqa: E402
...
def get_only_checkpoint_path(model_checkpoint_directory: Path) -> Path:
    ...
    if len(relevant_paths) != 1:
        raise FileNotFoundError(
            "Could not find the only model checkpoint "
...
semcor_jsonl_file_path = write_to_jsonl(
    wsd_sentence_generator(semcor_data_directory, GET_SENSE),
    DATA_PROCESSING_DIR,
    "semcor",
    overwrite=False,
)
...
token_sense_labels = semcor_dataset_splits.map(
    map_token_sense_labels,
    remove_columns=semcor_dataset_splits["train"].column_names,
    batched=False,
    fn_kwargs={"word_net_sense_getter": GET_SENSE},
    num_proc=8,
)
...
raganato_validation_jsonl_file_path = write_to_jsonl(
    wsd_sentence_generator(raganato_validation_data, GET_SENSE),
    DATA_PROCESSING_DIR,
    "raganato_validation",
    overwrite=False,
)
raganato_validation_dataset = load_dataset(
    "json", data_files=str(raganato_validation_jsonl_file_path)
)["train"]
validation_token_sense_labels = raganato_validation_dataset.map(
    map_token_sense_labels,
    remove_columns=raganato_validation_dataset.column_names,
    batched=False,
    fn_kwargs={"word_net_sense_getter": GET_SENSE},
    num_proc=8,
)
...
validation_token_sense_neg_labels = validation_token_sense_labels.map(
    map_negative_sense_ids,
    num_proc=8,
    fn_kwargs={
        "sense_id_key": "sense_labels",
        "word_net_lexicon": ENGLISH_WN,
        "lemma_key": "lemmas",
...
validation_token_sense_neg_labels_filtered = validation_token_sense_neg_labels.map(
    map_empty_removal_token_values,
    num_proc=8,
    fn_kwargs={
        "filter_key": "negative_labels",
        "aligned_keys": [
            "labels",
            "sense_labels",
            "lemmas",
...
validation_token_sense_neg_labels_filtered = (
    validation_token_sense_neg_labels_filtered.filter(
        filter_empty_values, num_proc=8, fn_kwargs={"key": "labels"}
    )
)

validation_token_definitions = validation_token_sense_neg_labels_filtered.map(
    map_to_definitions,
    num_proc=8,
    fn_kwargs={
        "word_net_lexicon": ENGLISH_WN,
        "sense_key": "sense_labels",
        "definition_key": "sense_definitions",
    },
)
validation_token_definitions = validation_token_definitions.map(
    map_to_definitions,
    num_proc=8,
    fn_kwargs={
        "word_net_lexicon": ENGLISH_WN,
        "sense_key": "negative_labels",
        "definition_key": "negative_definitions",
    },
)


validation_tokenized_definitions = validation_token_definitions.map(
    tokenize_key,
    fn_kwargs={
        "tokenizer": tokenizer,
        "text_key": "sense_definitions",
        "output_key_prefix": "positive",
        "add_word_ids": False,
    },
)
validation_tokenized_definitions = validation_tokenized_definitions.map(
    tokenize_key,
    fn_kwargs={
        "tokenizer": tokenizer,
        "text_key": "negative_definitions",
        "output_key_prefix": "negative",
        "add_word_ids": False,
    },
)
validation_tokenized_definitions = validation_tokenized_definitions.map(
...
test_batch_sampler = AscendingTokenNegativeExamplesBatchSampler(
    validation_tokenized_definitions,
    batch_size=1,
    positive_sample_key="positive_input_ids",
    negative_sample_key="negative_input_ids",
    random=False,
)
...
token_sense_neg_labels = token_sense_labels.map(
    map_negative_sense_ids,
    num_proc=8,
    fn_kwargs={
        "sense_id_key": "sense_labels",
...
token_sense_neg_labels_filtered = token_sense_neg_labels.map(
    map_empty_removal_token_values,
    num_proc=8,
    fn_kwargs={
        "filter_key": "negative_labels",
...
token_sense_neg_labels_filtered = token_sense_neg_labels_filtered.filter(
    filter_empty_values, num_proc=8, fn_kwargs={"key": "labels"}
)

token_definitions = token_sense_neg_labels_filtered.map(
    map_to_definitions,
    num_proc=8,
    fn_kwargs={
        "word_net_lexicon": ENGLISH_WN,
...
token_definitions = token_definitions.map(
    map_to_definitions,
    num_proc=8,
    fn_kwargs={
        "word_net_lexicon": ENGLISH_WN,
...
tokenized_definitions = token_definitions.map(
    tokenize_key,
    fn_kwargs={
        "tokenizer": tokenizer,
        "text_key": "sense_definitions",
...
tokenized_definitions = tokenized_definitions.map(
    tokenize_key,
    fn_kwargs={
        "tokenizer": tokenizer,
        "text_key": "negative_definitions",
...
columns_to_drop = [
    column_name
    for column_name in tokenized_definitions["train"].column_names
    if column_name not in columns_required
]
...
train_batch_sampler = AscendingTokenNegativeExamplesBatchSampler(
    train_dataset,
    batch_size=1,
    positive_sample_key="positive_input_ids",
    negative_sample_key="negative_input_ids",
...
validation_batch_sampler = AscendingTokenNegativeExamplesBatchSampler(
    validation_dataset,
    batch_size=1,
    positive_sample_key="positive_input_ids",
    negative_sample_key="negative_input_ids",
...
test_classifier = TokenSimilarityVariableNegatives(
    base_model_name,
    freeze_base_model=True,
    number_transformer_encoder_layers=2,
    transformer_encoder_hidden_dim=256,
...
model_checkpoint_callback = ModelCheckpoint(
    monitor=tracking_metric, save_top_k=1, mode="max", filename=checkpoint_file_name
)
...
best_token_model = TokenSimilarityVariableNegatives.load_from_checkpoint(
