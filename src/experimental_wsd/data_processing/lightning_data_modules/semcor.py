...
from experimental_wsd.config import DATA_PROCESSING_DIR, RaganatoEnglish
...
class VariableSemCorTraining(L.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        semcor_dataset = datasets.load_dataset(
            "json", data_files=str(semcor_jsonl_file_path)
        )
        ...
        flattened_sense_labels = token_negative_sense_labels.map(
            sample_to_a_sense, batched=True
        )
        # Remove samples that do not have a negative label
        filtered_flattened_sense_labels = flattened_sense_labels.filter(
            filter_empty_values, fn_kwargs={"key": "negative_labels"}
        )
    ...
    def setup(self, stage: str) -> None:
        if stage == "fit" or stage == "validate":
            train_validation_dataset = datasets.load_from_disk(
    ...
    def train_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        training_sampler = DescendingTokenSimilarityBatchSampler(
            self.train,
            batch_size=self.batch_size,
            similarity_sentence_key="label_definitions_input_ids",
            random=True,
        )
    ...
    def val_dataloader(self) -> torch.utils.data.DataLoader:
        ...
        validation_sampler = DescendingTokenSimilarityBatchSampler(
            self.validation,
            batch_size=self.batch_size,
            similarity_sentence_key="label_definitions_input_ids",
            random=False,
