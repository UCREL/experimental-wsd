...
from experimental_wsd.wordnet_utils import get_definition, get_negative_wordnet_sense_ids, get_normalised_mwe_lemma_for_wordnet
...
from experimental_wsd.evaluation.similarity_utils import load_keys, evaluate_macro_f1, evaluate_micro_f1
...
def evaluate_on_wordnet_dataset(
    ...
    with torch.inference_mode(mode=True):
        for wsd_sample in wsd_sentence_generator(dataset_directory,
                                                 wordnet_sense_getter):
            ...
            for annotation in wsd_sample.annotations:
                ...
                true_labels = [wordnet_sense_getter(label).id for label in annotation.labels]
                negative_labels = get_negative_wordnet_sense_ids(lemma, pos_tag, true_labels, wordnet, get_random_sense=False)
                ...
                tokenized_label_definitions = tokenizer(all_label_definitions, return_tensors="pt", padding=True)
                label_definitions_input_ids = tokenized_label_definitions.input_ids.unsqueeze(0).to(device=model.device)
                label_definitions_attention_mask = tokenized_label_definitions.attention_mask.unsqueeze(0).to(device=model.device)



                out = model.forward(text_input_ids, text_attention_mask, text_word_ids_mask, label_definitions_input_ids, label_definitions_attention_mask)
    ...
    if len(gold_keys) != len(prediction_keys):
        raise ValueError(f"The number of predictions ({len(prediction_keys)}) "
